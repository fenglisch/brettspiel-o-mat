...
var TEXTFILTER_KEYWORDS = [" ", "&#8203;Solo", "&#8203;2", "&#8203;3/4", "&#8203;5", "&#8203;6", "&#8203;7 oder mehr"]

// 2.) Text für Buttons
// Text on buttons
var TEXTFILTER_BUTTONTEXTS = ["Alle&nbsp;anzeigen", "Solo", "2", "3&nbsp;/&nbsp;4", "5", "6", "≥ 7"]
...
function mow_addon_textfilter_create_buttons() {
	...
	// schreibe Buttons
		...
		divContent = '<div class="row filterButtons">'
		...
		filterLabel.innerHTML = "&nbsp;Nach Spieler:innenanzahl filtern:";
		document.querySelector("#resultsAddonTop").appendChild(filterLabel);
	...
	document.querySelector(".filterButtons").querySelectorAll("button").forEach(button => {
		button.addEventListener("click", () => {
		// Wenn ein Eintrag ausgeklappt war, wird er beim Wegfiltern zugeklappt.
		// Verhindert Konflikt mit resultsByPartyAnswersToQuestion-Tabelle, wenn diese in die resultsShortTable eingefügt ist
			for (let i = 0; i < intParties; i++) {
				if (document.querySelector("#resultsShortPartyDescription"+i).style.display !== "none") {
					document.querySelector("#resultsShortParty"+i).click() 
				}
			}
		})
	})
