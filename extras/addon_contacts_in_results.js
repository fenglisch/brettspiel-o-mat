...
const CR_CONTACT_ACTIVE_TEL = 0

// Beschriftung der Buttons - unter der Ergebniszeile 
const CR_CONTACT_BUTTON_EMAIL = "Kontakt aufnehmen"
...
let CR_CONTACT_ADDRESS_EMAIL = "kontakt@mitwirk-o-mat.de"
...
const CR_CONTACT_SUBJECT_EMAIL_DATATAG =
"[Mitwirk-O-Mat] Interesse an eurem Verein ";
// 2. - Rückfall-Lösung falls kein DATA-EMAIL-Tag in der CSV angegeben wurde. 
const CR_CONTACT_SUBJECT_EMAIL_DEFAULT = 
"[Mitwirk-O-Mat] Interesse am Verein: ";

// E-Mailinhalt für die Kontaktaufnahme ...
// 1. - wenn ein DATA-EMAIL-Tag in der CSV-Datei vorhanden ist.
let CONTACT_TEXT_EMAIL_DATATAG =
"Hallo, \n\n\nich habe gerade den Mitwirk-O-Mat ausgefüllt und bin bei euch gelandet. \n\nBitte ruft mich doch mal an oder schreibt mir, so dass ich euch besser kennenlernen kann.\n\n\nViele Grüße\n\n\n";
// 2. - Rückfall-Lösung falls kein DATA-EMAIL-Tag in der CSV angegeben wurde.
let CONTACT_TEXT_EMAIL_DEFAULT =
"Guten Tag, \n\n\nich habe eben den Mitwirk-O-Mat auf eurer Website ausgefüllt. Der im Betreff genannte Verein hat dabei besonders mein Interesse geweckt. \n\nBitte schreibt mir doch mal, sodass ich mehr über den Verein und weitere Engagement-Angebote in Varel erfahren kann. \n\n\nViele Grüße\n\n\n";
...
function mow_addon_contacts_add_results_to_email_text(showParty,showPersonal) {
		...
		if (showPersonal > 0) {
			...
			for (i = 0; i <= (intQuestions-1); i++)
			{
				...
				if (questionPoints >= 99) statistics_text += `\n ${i + 1}. ${questionShort}: Übersprungen`;
				else if (questionPoints >= 1) statistics_text += `\n ${i + 1}. ${questionShort}: Zustimmung`;
				else if (questionPoints == 0) statistics_text += `\n ${i + 1}. ${questionShort}: Neutral`;
